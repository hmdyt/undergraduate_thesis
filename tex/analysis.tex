...
具体的には図\ref{fig:threhist}に示すような分布が得られ、図\ref{fig:adc_eff}のようにADC value のしきい値を800から1500まで1刻みで変更した時の検出効率を計算し, チャンネル毎にeffeciencyが大きく下がらない程度のしきい値を探すことでしきい値を決定した.
...
図\ref{fig:anal:bg01}は\ref{sec:eventcut_val}節で使用したシミュレーションデータから, イベントセレクションがパイオンイベントと間違えたイベントを可視化したものの例である.図\ref{fig:anal:bg01}に示すようなミューオンが検出器内の電子を弾くイベントが支配的であった。図\ref{fig:electron_degrere}, \ref{fig:pion_degrere}はそれぞれシミュレーションによって得られたミューオンによって弾き出された電子とパイオンの角度分布である。角度は上空方向を正としたベクトルと粒子の運動量ベクトルの内積によって定義される。
図\ref{fig:electron_degrere}, \ref{fig:pion_degrere}からわかるように電子は角度の小さい領域への散乱はパイオンに比べて少ないので、この事実を用いてパイオンのみを選ぶイベントセレクションが実現できると考えられる。
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    \begin{tabular}{cc}
        \begin{minipage}[t]{0.45\hsize}
            \centering
            \includegraphics[width=7.0cm]{img/electron_degree.png}
            \caption{電子の散乱角分布}
            \label{fig:electron_degrere}
        \end{minipage}
        \begin{minipage}[t]{0.45\hsize}
            \centering
            \includegraphics[width=7.0cm]{img/pion_degree.png}
            \caption{パイオンの散乱角分布}
            \label{fig:pion_degrere}
        \end{minipage}
    \end{tabular}
\end{figure}